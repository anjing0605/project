...
class NodeScorePathBaseline:
    ...
    def run(
        ...
        if shared_base_metrics is None:
            ...
            print(
                f"[node_score] base_metrics computed locally in {time.perf_counter() - t0:.2f}s: {base_metrics}",
                flush=True,
            )
        else:
            base_metrics = dict(shared_base_metrics)
            print(f"[node_score] using shared_base_metrics: {base_metrics}", flush=True)
