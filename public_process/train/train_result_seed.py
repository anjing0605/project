...
class SlimRunDataset(Dataset):
    ...
    def _process_single(self):
        ...
        data = Data(
            ...
            rank_sir=torch.tensor(rank_sir, dtype=torch.float),
...
def ranking_loss(pred, rank, mask, margin=0.2):
    ...
    i = torch.randint(0, N, (8000,), device=p.device)
    j = torch.randint(0, N, (8000,), device=p.device)

    sign = torch.sign(r[j] - r[i])

    valid = sign != 0

    diff = p[i] - p[j]

    loss = F.relu(margin - sign * diff)
...
def topk_loss(pred, target, mask, ratio=0.2):
    ...
    top_idx = torch.topk(-target, k).indices
    other_idx = torch.topk(target, N-k).indices

    # 随机采样
    i = top_idx[torch.randint(0, k, (10000,))]
    j = other_idx[torch.randint(0, N-k, (10000,))]

    diff = pred[i] - pred[j]

    loss = F.relu(0.05 - diff)
...
def train_one_epoch(model, loader, optimizer, device, config):
    ...
    for data in loader:
        ...
        loss_rank = ranking_loss(pred, data.rank_sir, data.train_mask)

        loss_reg = regression_loss(pred, data.rank_sir.float()/data.rank_sir.max(), data.train_mask)

        loss_topk = topk_loss(pred, data.rank_sir, data.train_mask)

        loss = (
            config["w_rank"] * loss_rank +
...
def topk_overlap(pred, target, nodes, k):

    # 构造 (node, score)
    pred_pairs = list(zip(nodes, pred))
    true_pairs = list(zip(nodes, target))

    # 排序
    pred_pairs.sort(key=lambda x: x[1], reverse=True)
    true_pairs.sort(key=lambda x: x[1], reverse=True)

    # 取TopK节点ID
    pred_nodes = {n for n, _ in pred_pairs[:k]}
    true_nodes = {n for n, _ in true_pairs[:k]}

    overlap = len(pred_nodes & true_nodes)
...
def evaluate(model, loader, device, split="val"):
    ...
    for data in loader:
        ...
        pred = pred_sir[mask].cpu()
        true = (-data.rank_sir[mask]).cpu()

        # ⭐ 保留原始节点 ID
        node_ids = torch.arange(data.num_nodes,device=data.x.device)[mask].cpu()
    ...
    nodes = torch.cat(all_nodes).numpy()
    assert len(nodes) == len(set(nodes)), "Node IDs duplicated in evaluation"

    # 排序指标
    sp, kd = rank_metrics(pred, true)

    # TopK
    top5 = topk_overlap(pred, true, nodes, 5)
    top10 = topk_overlap(pred, true, nodes, 10)
    top20 = topk_overlap(pred, true, nodes, 20)
...
if __name__ == "__main__":
    ...
    for name in datasets:
        ...
        out_root = f"{BASE}/{name}_struct/result"
        seed_results = []
        ...
        for seed in seeds:
            ...
            for epoch in range(1, max_epoch + 1):
                ...
                if epoch % 20 == 0:
                    print(
                        ...
                        f"T5 {metrics['top5']:.3f} | "
                        f"T10 {metrics['top10']:.3f}|"
                        f"T20 {metrics['top20']:.3f}"
                    )
            ...
            def export_scores():
                ...
                for batch in loader:
                    batch = batch.to(device)
                    score = model(batch).cpu().numpy().reshape(-1)
