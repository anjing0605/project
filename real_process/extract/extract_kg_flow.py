...
'''
python3 scratch/project/scripts/extract_kg_flow.py \
  --output_dir scratch/project/path_planning/global_training_data \
  --num_runs 150 

'''
'''
python3 scratch/project/path_planning/global_training_data/build_labels_from_interventions.py \
  --base_dir scratch/project/path_planning/global_training_data \
  --mode upgrade --apply_mask --mask_frac 0.5 --seed 123
'''
import os
import re
import glob
...
def process_single_run(run_id):
    ...
    for node in sorted(all_nodes):
        ...
        # ✅ 修正：用 run 级观测时间窗口作为分母（避免 dur_sum_s 并行流相加导致吞吐系统性偏小）
        throughput_mbps = (rx_bytes * 8.0) / max(sim_time_s, 1e-9) / 1e6
...
if __name__ == "__main__":
    ...
    print(f"成功处理的 runs: {processed_runs}")

    # 3) 生成 KG
    for rid in processed_runs:
        rdir = os.path.join(args.output_dir, f"run_{rid}")
        generate_kg_triples(rdir)

    print("✅ 节点特征 + KG 已全部生成完毕")
